...
C[<b>Solution Alpine.js</b>] -.-> |Apporte|D[Réactivité client] --> |Permet| E[Interactions riches] -.-> |Offre| F[UX moderne]
...
_Charge les tâches depuis l'API._
...
_Ajoute une nouvelle tâche via l'API._
...
_Computed property pour filtrer les tâches._
...
sequenceDiagram
    ...
    DB-->>API: Données
    API-->>A: JSON Response
    A-->>U: Interface mise à jour

    Note over U,DB: Interaction côté client
    U->>A: Tape dans le formulaire
    A->>A: x-model réactivité
    A-->>U: Validation temps réel

    U->>A: Clique "Ajouter"
    A->>A: @click="addTask()"
    A->>API: POST /api/tasks
    API->>DB: INSERT INTO tasks
    DB-->>API: Nouvelle tâche
    API-->>A: JSON Response
    A->>A: Mise à jour du state
    A-->>U: DOM mis à jour automatiquement
```
