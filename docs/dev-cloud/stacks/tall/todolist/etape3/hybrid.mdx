...
:::danger A prendre en compte immédiatement
**Parcours Livewire + Alpine.js ( _Hybride - Concept avancé_ 🔴 )**<br />
...
E[<b>Solution Hybride</b>] -.-> |Combine|F[Livewire logique] & G[Alpine.js interactions] -.-> |Résultat| H[Expérience optimale]
...
### Qu'est-ce que la `Communication Livewire ↔ Alpine.js`
...
**Livewire → Alpine.js** : Via `dispatch()` et `x-on`
...
```php
// Ajouter une nouvelle tâche - Logique Livewire
...
```php
// Supprimer une tâche - Logique Livewire
...
```php
// Computed property pour les tâches - Logique Livewire
...
### 1. Communication Livewire → Alpine.js
...
### 2. Communication Alpine.js → Livewire
...
>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-xl font-semibold">🔍 Recherche et filtres</h2>
            <div class="text-sm text-gray-500" x-show="hasActiveFilters">
                <span x-text="searchStats.filtered"></span> /
                <span x-text="searchStats.total"></span> tâches
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md" x-ref="taskList">
        <div class="p-4 border-b">
            <h2 class="text-xl font-semibold">
                📋 Mes Tâches (<span x-text="filteredTasks.length"></span>)
            </h2>
        </div>
...
sequenceDiagram
    ...
    A->>A: Filtre instantané (côté client)
    A-->>U: Résultats immédiats

    U->>L: Clique "Ajouter tâche"
    L->>S: Validation + logique métier
    S->>DB: Persistance des données
    DB-->>S: Succès
    S->>L: Tâche créée
    ...
    U->>A: Drag & drop tâche
    A->>A: Animation immédiate
    A->>L: Appel $wire.reorderTasks()
    L->>S: Mise à jour ordre
